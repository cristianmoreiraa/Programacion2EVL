...
public class Metodos {
    ...
    public void engadir(File e, ArrayList<Libros> lista,int nEngadir){
        ...
        while(i < nEngadir){
            try{
                String nombre;
                nombre = PedirDatos.pedirString("Teclee o titulo do " + (i+1) + " libro");
                
                if(lista.size()>0){
                    for(int z = 0; z < lista.size(); z++){
                        if(lista.get(z).getNome().equalsIgnoreCase(nombre)){
                            nombre = PedirDatos.pedirString("O titulo non pode ser igual a uno existente, intentelo de nuevo");
                            z=0;
                        }
                    }
                }
                String autor = PedirDatos.pedirString("Teclee o autor do " + (i+1) + " libro");
                float precio = PedirDatos.pedirFloat("Teclee o precio do " + (i+1) + " libro");
                int unidades = PedirDatos.pedirInt("Teclee as unidades a haber no libro: " + nombre);
    ...
    public void modificarPrecio(File e,ArrayList<Libros> lista, String titulo){
        for(int i = 0; i < lista.size(); i++){
            if(titulo.equalsIgnoreCase(lista.get(i).getNome())){
                float precio;
                precio = PedirDatos.pedirFloat("Precio anterior = " + lista.get(i).getPrecio() + "\nTeclee el nuevo precio");
